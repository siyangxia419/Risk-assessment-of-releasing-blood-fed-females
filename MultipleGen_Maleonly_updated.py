...
N_release = 0  # Number of released females: when releasing only males, no females were released
...
parameterarray = np.loadtxt("Manual_Maleonly", delimiter=",")
...
def one_mosquito(humaninf, trinary, vcmu, vcsigma):
    ...
    for x in range(0, 9):
        ...
        for i in np.nditer(randn):  # determine whether each bite infect the mosquito
            if i < (humaninf * vc):  # hi * vc is the probability of the mosquito got affected biting any human
    ...
    for x in range(0, 10):
        ...
        mosq_roulette = np.random.binomial(bitesincyc, (1 - humaninf))
...
while z < N_simulation:  # Z is the number of simluations
    ...
    for J in range(0, 10):  # the 10 generations
        ...
        if J > 0:  # makes HI proportion of gen 1 infections
            hii = hi * ((GenerationMatrix[z, J - 1] +
                         GenerationMatrix[z, J + 10 - 1]) / GenerationMatrix[z, 0])
        else:
            hii = hi

        print("human infection proportion:", hii)
        ...
        while x < N_mosquito:  # adjust this for number of wild mosquitos
            ...
            wild = one_mosquito(hii, 0, vcw * VC_genX_factor, sdVCw)
        ...
        while xx < (N_release * releaseratio):  # adjust this for number of released mosquitos
            if xx < (N_release * releaseratio * fed):
                ...
                released = one_mosquito(hii, 1, vcr, sdVCr)
            else:
                # the released mosquitoes that faild to fed in the lab
                released = one_mosquito(hii, 2, vcr, sdVCr)
...
np.savetxt(f'simulation_Maleonly_MN_Total_{name:03.0f}.csv', stochastic_array, delimiter=",")
np.savetxt(f'simulation_Maleonly_MN_Gens_{name:03.0f}.csv', GenerationMatrix, delimiter=",")
